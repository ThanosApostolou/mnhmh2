...
export class DirectMaterialBorrowersPage extends React.Component<Record<string, never>, DirectMaterialBorrowersPageState> {
    ...
    fetchData(): void {
        ...
        DirectMaterialBorrower.listFromApi(this.cancelTokenSource).then((data: any) => {
            this.setState({dmbs: data});
            this.setState({rows: DirectMaterialBorrower.getRows(this.state.dmbs)});
        }).catch((error) => {
            this.setState({dmbs: null});
            this.setState({rows: []});
    ...
    onRowSelected(params: GridRowSelectedParams): void {
        if (this.state.dmbs && this.state.dmbs !== null && this.state.dmbs.length > 0) {
            this.setState({selectedDmb: this.state.dmbs[params.data.AA - 1]});
        } else {
            this.setState({selectedDmb: null});
        }

        console.log("manager", this.state.selectedDmb);
    }
...
export interface DirectMaterialBorrowersPageState {
    dmbs: DirectMaterialBorrower[];
    selectedDmb: DirectMaterialBorrower;
    rows: GridRowsProp;
    loading: boolean;
    search: string;
    error: any;
